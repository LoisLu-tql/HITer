...
class ThirdLevel1:
    def __init__(self):
        ...
        self.start_set = True
        self.level = 0
        self.info = info.Info('third_level_1.json')
        self.setup_background()
        self.load_map_info()
        self.save_map()
        self.arrive_set = True
        self.full_blood = True
    ...
    def update(self, surface, keys, press):
        ...
        if self.start_set:
            ...
            self.enemies_pos_2 = [(200, 500), (200, 550)]
            functions.setup_enemy(3, 'LAB_robot_B', self.player, self.enemies, self.enemies_pos_1)
        ...
        if not player_passageway_type and not player_obstacle_type:
            ...
            if len(self.enemies) == 0 and self.player.health_point > 0 and len(self.drop_items) == 0 and\
                ...
                if player_passageway_type == 'go_left' and self.graph.hasEdge(self.level, self.level):
                    pass
                    ...
                    pass
                elif player_passageway_type == 'go_right' and self.graph.hasEdge(self.level, self.level + 3):
                    functions.update_blood_stored(3, self.player)
                    self.arrive_set = True
                    self.finished = True
