...
class ThirdLevel4:
    def __init__(self):
        ...
        self.start_set = True
        self.level = 3
        self.info = info.Info('third_level_4.json')
    ...
    def setup_background(self):
        self.background = setup.graphics['map_3.4']
    ...
    def update(self, surface, keys, press):
        ...
        if self.start_set:
            ...
            self.enemies_pos_1 = [(550, 200), (600, 200), (650, 200)]
            self.enemies_pos_2 = [(600, 700), (650, 700)]
            functions.setup_enemy(3, 'LAB_robot_W', self.player, self.enemies, self.enemies_pos_1)
            functions.setup_enemy(2, 'LAB_robot_G', self.player, self.enemies, self.enemies_pos_2)
        ...
        if not player_passageway_type and not player_obstacle_type:
            ...
            if len(self.enemies) == 0 and self.player.health_point > 0 and len(self.drop_items) == 0 and\
                ...
                if player_passageway_type == 'go_down':
                    file_name_2 = 'third_level_memory.txt'
                    file_path_2 = os.path.join('source/states/thirdlevel', file_name_2)
                    with open(file_path_2, 'r') as f1:
                        a = f1.read()
                        if a == '4':
                            ...
                            self.arrive_set = True
                            functions.update_state_now('tip10')
                            self.finished = True
                            self.next = 'tip10'
                        else:
                            ...
                            functions.update_player_pos(self.player)
                            surface.blit(self.player.image, self.player.rect)
                elif player_passageway_type == 'go_left' and self.graph.hasEdge(self.level, self.level - 3):
                    functions.update_blood_stored(3, self.player)
                    self.arrive_set = True
                    self.finished = True
                    self.next = 'third_level_1'
                elif player_passageway_type == 'go_right' and self.graph.hasEdge(self.level, self.level):
                    pass
                elif player_passageway_type == 'go_up' and self.graph.hasEdge(self.level, self.level - 1):
                    functions.update_blood_stored(3, self.player)
                    self.arrive_set = True
                    self.finished = True
