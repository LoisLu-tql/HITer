...
class FirstLevel1:
    ...
    def setup_background(self):
        ...
        self.background = pygame.transform.scale(self.background, (int(rect.width * 1.47),
                                                                   int(rect.height * 1.53)))
        self.background_rect = self.background.get_rect()

        self.viewport = setup.screen.get_rect()

    def load_map_info(self):
        file_name = 'first_level_1.json'
    ...
    def update(self, surface, keys, press):
        ...
        if self.start_set:
            ...
            self.enemies_pos_1 = [(180, 300), (180, 350), (180, 400)]
            self.enemies_pos_2 = [(700, 400), (700, 450)]
        ...
        if not player_passageway_type and not player_obstacle_type:
            ...
            if len(self.enemies) == 0 and self.player.health_point > 0 and len(self.drop_items) == 0 and\
                ...
                if player_passageway_type == 'go_left' and self.graph.hasEdge(self.level, self.level):
                    ...
                    pass
                elif player_passageway_type == 'go_down' and self.graph.hasEdge(self.level, self.level + 2):
                    functions.update_blood_stored(1, self.player)
                    self.arrive_set = True
                    self.finished = True
