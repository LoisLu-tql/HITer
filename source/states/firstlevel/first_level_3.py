...
class FirstLevel3:
    def __init__(self):
        ...
        self.start_set = True
        self.level = 2
        self.info = info.Info('first_level_1.json')
    ...
    def setup_background(self):
        self.background = setup.graphics['map_1.3']
    ...
    def update(self, surface, keys, press):
        ...
        if self.start_set:
            ...
            self.enemies_pos_1 = [(300, 300), (200, 350), (200, 390)]
            self.enemies_pos_2 = [(200, 440), (300, 250)]
            functions.setup_enemy(3, 'PA_robot_R', self.player, self.enemies, self.enemies_pos_1)
            functions.setup_enemy(2, 'PA_robot_G', self.player, self.enemies, self.enemies_pos_2)
        ...
        if not player_passageway_type and not player_obstacle_type:
            ...
            if len(self.enemies) == 0 and self.player.health_point > 0 and len(self.drop_items) == 0 and\
                ...
                if player_passageway_type == 'go_down':
                    ...
                    with open(file_path_2, 'r') as f1:
                        ...
                        if a == '3':
                            ...
                            functions.update_state_now('tip4')
                            self.arrive_set = True
                            self.finished = True
                            self.next = 'tip4'
                        else:
                ...
                elif player_passageway_type == 'go_left' and self.graph.hasEdge(self.level, self.level - 1):
                    functions.update_blood_stored(1, self.player)
                    self.arrive_set = True
                    self.finished = True
                    self.next = 'first_level_2'
                elif player_passageway_type == 'go_right' and self.graph.hasEdge(self.level, self.level):
                    pass
                elif player_passageway_type == 'go_up' and self.graph.hasEdge(self.level, self.level - 2):
                    functions.update_blood_stored(1, self.player)
                    self.arrive_set = True
                    self.finished = True
